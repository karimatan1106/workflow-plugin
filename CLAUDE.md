...
大規模プロジェクトでは、以下のClean Architecture + DDDに基づく構造を推奨します。
...
│   ├── use-cases/        # ユースケース実装
│   ├── commands/         # コマンド（CQRS Write操作）
│   ├── queries/          # クエリ（CQRS Read操作）
│   ├── ports/            # 入出力ポート定義
│   └── services/         # アプリケーションサービス
│
├── infrastructure/       # 外部依存の実装（依存性逆転）
│   ├── database/         # データベース接続・リポジトリ実装
│   ├── external-apis/    # 外部API連携
│   ├── messaging/        # メッセージング（キュー等）
│   └── config/           # 設定ファイル
│
└── presentation/         # UI/API層（Application層経由でアクセス）
    ├── controllers/      # コントローラー
    ├── dtos/             # データ転送オブジェクト
    ├── middleware/       # ミドルウェア
    └── views/            # ビュー（該当する場合）
...
- **Domain層**: 外部依存禁止。純粋なビジネスロジックのみ
- **Application層**: Domain層のみに依存。ユースケースを実装
- **Infrastructure層**: Domain/Application層の抽象（インターフェース）に依存（依存性逆転原則）
- **Presentation層**: Application層を通じてのみドメインにアクセス

### フェーズ別Clean Architecture観点

| フェーズ | 考慮すべき観点 |
|---------|---------------|
| requirements | ドメイン境界の特定、ユビキタス言語（用語集）の定義 |
| planning | 層構成の決定、モジュール分割、集約単位の検討 |
| state_machine | ドメインエンティティの状態遷移設計 |
| flowchart | ユースケースフロー、レイヤー間通信パターン |
| test_design | 層ごとのテスト戦略（Domain:単体、Application:統合、E2E） |
| implementation | 依存性逆転原則の適用、ポート/アダプターパターン |
