...
## agent team活用（並列レビューパターン）

### design_reviewフェーズでの活用

| 専門家 | レビュー対象 | 観点 |
|--------|-------------|------|
| セキュリティ | threat-model.md, spec.md | OWASP Top 10、認証・認可、データ暗号化 |
| パフォーマンス | spec.md, state-machine.mmd | N+1クエリ、キャッシュ戦略、非同期処理 |
| UI/UX | ui-design.md, flowchart.mmd | WCAG 2.1、レスポンシブ対応、ユーザージャーニー |

### code_reviewフェーズでの活用

| 専門家 | 観点 |
|--------|------|
| コード品質 | SOLID原則、DRY原則、命名規則の一貫性 |
| テストカバレッジ | test-design.mdの全テストケースカバー、エッジケース、モックの適切な使用 |

### agent teamのベストプラクティス

1. **専門性の分離**: セキュリティ、パフォーマンス、UI/UX等の専門家を分離
2. **並列実行**: 1つのメッセージで複数のTask呼び出しを行い、同時にレビュー
3. **レビュー結果の統合**: 各subagentの結果をメインClaudeが統合
4. **承認プロセス**: 全subagentのレビュー完了後、ユーザーに承認を求める
