...
Task({
  prompt: `
    ...
    **回避策:** 同じラベル構造を3回以上使う場合、各行に固有の情報を含める
    ...
    - コードフェンスの内外を問わず角括弧の使用は禁止される（シングルバックティック内のインラインコードは除去されるため安全）
    - コードフェンス内に正規表現を記述する場合は、文字クラスの代わりに散文で説明すること（例: 「ハイフンまたはアスタリスクの文字クラス」）
    - 配列の要素を参照する場合は「先頭要素」「2番目の要素」という表記で角括弧を回避できる
