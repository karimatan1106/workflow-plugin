  ...
  parallel_design: {
    ...
    subPhases: {
      ...
      ui_design: {
        ...
        subagentTemplate: '# ui_designフェーズ\n\n## タスク情報\n- ユーザーの意図: ${userIntent}\n- 出力先: ${docsDir}/\n\n## 入力\n${docsDir}/spec.md を読み込んでください。\n\n## 作業内容\nUI設計を作成してください。\n\n## 成果物の永続化についてのご説明\n\n本フェーズで作成した .mmd ファイル（例: state-machine.mmd, flowchart.mmd）および .md ファイル（ui-design.md等）は、デフォルトの docsDir（docs/workflows/{taskName}/）に配置されます。\ndocs/workflows/ 配下のファイルは .gitignore により自動的に除外されるため、git 追跡対象外となります。\nこれらのファイルの永続化（docs/spec/diagrams/ 等への移動）は docs_update フェーズの責任です。\n本フェーズでは docsDir への配置をデフォルトとしてください。\n\n## 出力\n${docsDir}/ui-design.md\n\n## ★ワークフロー制御ツール禁止★\n\nこのsubagentの責任範囲は上記の作業内容のみである。\nサブフェーズ完了宣言・フェーズ遷移はOrchestratorの専権事項であり、以下のMCPツールは絶対に呼び出してはならない:\n- workflow_next（フェーズ遷移）\n- workflow_approve（レビュー承認）\n- workflow_complete_sub（サブフェーズ完了宣言）— Orchestratorがサブフェーズ完了を宣言するためのものであり、subagentが呼び出してはならない\n- workflow_start（タスク開始）\n- workflow_reset（リセット）\n作業が完了した後は、速やかに処理を終了してOrchestratorに制御を返すこと。',
      },
    },
  },
  design_review: {
...
- ★★出力パス確認★★: 成果物は必ず ${docsDir}/ に保存すること。上記パス以外への保存は禁止。`);
  ...
  qualitySection += `\n### 角括弧プレースホルダー禁止\n`;
  qualitySection += `` + '`' + `[#変数名#]` + '`' + `、` + '`' + `[#パス#]` + '`' + ` 等のハッシュ記号付き角括弧プレースホルダーは使用禁止です。通常の角括弧はコードフェンス外でも使用可能です。\n`;
  qualitySection += `許可される角括弧: ${rules.bracketPlaceholderInfo.allowedKeywords.join('、')}\n`;
  qualitySection += `禁止対象は ` + '`' + `[#xxx#]` + '`' + ` 形式（ハッシュ記号付きプレースホルダー）のみである。コードフェンス内の行はextractNonCodeLinesにより検出から除外されるため、コードフェンス内であれば配列アクセス記法や正規表現の文字クラス表記を安全に記述できる\n`;
  qualitySection += `コードフェンス外でも通常の角括弧・配列アクセス記法・Markdownリンク記法は使用可能であり、ハッシュ記号付きプレースホルダー形式のみが禁止対象となる\n`;
  qualitySection += `\n正規表現パターンの記述:\n`;
  ...
  const errorPatterns: Array<{ patterns: string[]; messages: string[] }> = [
    {
      ...
      messages: [
        '`[#xxx#]` 形式のハッシュ記号付きプレースホルダーを検出した箇所を特定し、波かっこ形式または具体的な値に変更してください。通常の角括弧・配列アクセス記法・Markdownリンク記法はコードフェンス外でも使用可能です',
